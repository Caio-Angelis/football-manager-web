...
import { getCrestColors, getTeamBadge, getTeamInitials, getTeamTier, type TeamTier } from '../../utils/teamColors';
...
/** Team crest used across topbars/cards. Renders the official badge image when
 *  available, falling back to a procedural SVG shield so every club has a
...
export const TeamCrest: React.FC<TeamCrestProps> = ({ name, reputation, tier, size = 48, className }) => {
  ...
  const badgeSrc = getTeamBadge(name);
  const [imgError, setImgError] = useState(false);
  const showImage = badgeSrc && !imgError;
