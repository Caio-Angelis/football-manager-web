...
8. **PRNG Determinística (mulberry32):** Todas as chamadas de `Math.random()` no motor de partida foram substituídas por `_matchRng()`. Quando uma `seed` é fornecida (via `initLiveMatchState` ou `simulateFullMatch`), o motor usa uma PRNG mulberry32 determinística — mesma seed sempre produz o mesmo jogo. O estado do PRNG (`rngState`) é salvo no `LiveMatchState` a cada minuto, permitindo pausar e retomar a simulação exatamente de onde parou. Quando nenhuma seed é passada, o motor usa `Math.random()` (não-determinístico, backward-compatible). Isso habilita: replay de partida, testes determinísticos e online seguro (servidor autoritário, cliente reproduz).
